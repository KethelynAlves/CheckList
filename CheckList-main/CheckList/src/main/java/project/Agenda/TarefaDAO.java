...
public class TarefaDAO {
    ...
    public void acionarTarefa(String descricao) throws SQLException{
        String sql = "INSERT INTO tarefa(descricao,data_inicio,concluido) VALUES (?, strftime('%d-%m-%y %H-%M', 'now'), 0)"//tem que arrumar isso!! da data no caso;
    ...
    public void alterarStatus(int idTarefa, boolean status) throws SQLException{
        String sql = "UPDATE tarefa SET concluido = ?, data_alteracao = data_alteracao = date WHERE idTarefa=?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)){
            stmt.setInt(1, status?0:1);
    ...
    public List<Tarefa> listarTarefas(String casos) throws SQLException{
        ...
        String sql = switch (casos.toLowerCase()){
            case "pendentes" -> "SELECT * FROM tarefa WHERE concluido = 1";//logica parece trocada ksks
